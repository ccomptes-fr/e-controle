...
class DeleteViewSet(viewsets.ViewSet):
    ...
    def delete_control(self, request, pk):
        ...
        if control not in request.user.profile.user_controls("demandeur"):
            ...
            raise e
        if (
            control in request.user.profile.user_controls("demandeur")
            and control.is_deleted
        ):
            e = PermissionDenied(
        ...
        soft_delete_signal.send(sender=Control, session_user=request.user, obj=control)
