...
class Prefixer(object):
    ...
    def make_file_prefix(self):
        return (
            f"Q{self.questionnaire_num:02}-T{self.theme_num:02}-{self.question_num:02}"
        )
...
class PathBuilder(object):
    def __init__(self, file_object, filename):
        ...
        if hasattr(file_object, "question"):
            control_folder = (
                file_object.question.theme.questionnaire.control.reference_code
                or f"CONTROLE-{file_object.question.theme.questionnaire.control.id}"
            )
            ...
            control_folder = (
                file_object.questionnaire.control.reference_code
                or f"CONTROLE-{file_object.questionnaire.control.id}"
            )
    ...
    def get_questionnaire_pj_file_path(self):
        question_path = os.path.join(
            self.questionnaire_path, "PIECES-JOINTES-QUESTIONNAIRE"
        )
        return os.path.join(question_path, self.filename)
