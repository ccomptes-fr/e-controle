    ...
    def perform_update(self, serializer):
        ...
        if not serializer.validated_data["is_deleted"]:
            ...
            raise serializers.ValidationError(
                "Vous ne pouvez sortir un fichier réponse de la corbeille."
            )

        if instance.is_deleted:
            raise serializers.ValidationError(
                "Vous ne pouvez mettre à la corbeille un fichier qui y est déja."
            )
...
class QuestionnaireViewSet(
    ...
    def __create_or_update(self, request, save_questionnaire_func, is_update):
        ...
        if is_update:
            ...
            def log_delete(saved_object):
                self.__log_action(
                    request.user, "deleted", saved_object, saved_qr.control
                )

            self.__delete_objects_not_in_request_data(
                qr_in_db=saved_qr,
                themes_request_data=validated_themes_and_questions,
                log_delete_func=log_delete,
            )

        self.__save_themes_and_questions(
            qr_in_db=saved_qr,
            themes_request_data=validated_themes_and_questions,
            log_func=log,
        )
        ...
        response.data = control_serializers.QuestionnaireSerializer(
            instance=saved_qr
        ).data
        if not is_update:
    ...
    def create(self, request, *args, **kwargs):
        save_questionnaire_func = partial(
            super(QuestionnaireViewSet, self).create, request, *args, **kwargs
        )

        return self.__create_or_update(
            request, save_questionnaire_func, is_update=False
        )
    ...
    def __validate_all(self, request, verb, questionnaire_in_db=None):
        ...
        :return:
        """
        serializer = control_serializers.QuestionnaireUpdateSerializer(
            questionnaire_in_db, data=request.data
        )
    ...
    def __delete_objects_not_in_request_data(
        self, qr_in_db, themes_request_data, log_delete_func
    ):
        """
        ...
        theme_ids_in_request = [
            theme_request_data.get("id") for theme_request_data in themes_request_data
        ]
        ...
        for theme_request_data in themes_request_data:
            ...
            theme_in_db = __find_child_theme_by_id(
                qr_in_db, theme_request_data.get("id")
            )
            ...
            question_ids_in_request = [
                question_request_data.get("id")
                for question_request_data in questions_request_data
            ]
    ...
    def __save_themes_and_questions(self, qr_in_db, themes_request_data, log_func):
        ...
        for theme_request_data in themes_request_data:
            theme_in_db = self.__find_child_obj_by_id(
                qr_in_db, theme_request_data.get("id"), Theme
            )
            theme_serializer = control_serializers.ThemeSerializer(
                theme_in_db, data=theme_request_data
            )
