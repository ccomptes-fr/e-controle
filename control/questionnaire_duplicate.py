...
class QuestionnaireDuplicateMixin(object):
        ...
        for theme in original_questionnaire.themes.all():
            ...
            for question in original_theme.questions.all():
                ...
                for question_file in original_question.question_files.all():
                    original_question_file = QuestionFile.objects.get(
                        pk=question_file.pk
                    )
                    new_file = File(
                        original_question_file.file,
                        name=original_question_file.basename,
                    )
    ...
    def get_controls_to_copy_to(self, questionnaire):
        return Control.objects.filter(title=questionnaire.control.title).exclude(
            id=questionnaire.control.id
        )
    ...
    def do_megacontrol(self, questionnaire):
        ...
        for control_to_copy_to in controls_to_copy_to:
            created_questionnaire = self.copy_questionnaire(
                questionnaire, control_to_copy_to
            )
    ...
    def megacontrol_admin_action(self, request, queryset):
        ...
        questionnaire_id = ids_of_questionnaires_to_copy[
            0
        ]  # todo deal with more than 1 questionnaire
        return HttpResponseRedirect(
            reverse("megacontrol-confirm", args=[questionnaire_id])
        )
