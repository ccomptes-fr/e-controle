...
class CCDomainController(BaseDomainController):
  ...
  def basic_auth_user(self, realm, user_name, password, environ):
    ...
    def get_samaccount_from_env(environ):
      """
      Extract the SamAccount from the HTTP_X_FORWARDED_USER environment variable.
      The HTTP_X_FORWARDED_USER environment variable was set by the webserver when it received the request
      from the user.

      The format for HTTP_X_FORWARDED_USER is samaccount@CCOMPTES.FR.
      ...
      """
      username = environ['HTTP_X_FORWARDED_USER']
    ...
    try:
      cache_email_key = f"{samaccount}_email"
      cache_user_access_key = f"{samaccount}_{realm}"

      email_user = cache.get(cache_email_key)
      if not email_user:
        email_user = get_email_from_samacount(samaccount)
        cache.set(cache_email_key, email_user, settings.WEBDAV_CACHE_TIMEOUT)

      user_access = cache.get(cache_user_access_key)
      if not user_access:
        user = get_django_user_from_email(email_user)
        user_access = {"can_user_access_realm": can_user_access_realm(user, realm)}
        cache.set(cache_user_access_key, user_access, settings.WEBDAV_CACHE_TIMEOUT)

      return user_access["can_user_access_realm"]
