...
class CCDomainController(BaseDomainController):
    ...
    def basic_auth_user(self, realm, user_name, password, environ):
        ...
        def get_email_from_samacount(samaccount):
            ...
            if conn.bind():
                ...
                if len(conn.entries) == 1:
                    ...
                    raise Exception(
                        f"sAMAccount name {samaccount} not found in LDAP server"
                    )
                else:
                    raise Exception(
                        f"sAMAccount name has several email addresses linked to.({samaccount})"
                    )
            else:
        ...
        def can_user_access_realm(django_user, realm):
            ...
            if (
                django_user.profile.controls.filter(reference_code=realm).exists()
                or realm == ""
            ):
